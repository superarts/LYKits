...
- (void)calendarViewAdd:(CKCalendarView*)view
{
    ...
    if ([[self delegate] respondsToSelector:@selector(calendarViewAdd:)]) {
        [[self delegate] calendarViewAdd:view];
    }
}

- (void)calendarViewAction:(CKCalendarView*)view
{
    if ([self isEqual:[self delegate]]) {
        return;
    }
    ...
    if ([[self delegate] respondsToSelector:@selector(calendarViewAction:)]) {
        [[self delegate] calendarViewAction:view];
    }
}

- (void)calendarViewTrash:(CKCalendarView*)view
{
    if ([self isEqual:[self delegate]]) {
        return;
    }
    ...
    if ([[self delegate] respondsToSelector:@selector(calendarViewTrash:)]) {
        [[self delegate] calendarViewTrash:view];
    }
}
