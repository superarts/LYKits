...
- (void) viewWillAppear:(BOOL)animated {
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onDismiss)];
    [self.imageView addGestureRecognizer:tap];
	UIImage* another_image = [[UIImage alloc] initWithCGImage:self.liftedImageView.image.CGImage];
#endif
...
- (void) onDismiss {
    [self dismissViewControllerAnimated:YES completion:^(void) {
		...
		[self.delegate performSelector:@selector(fullscreen_dismiss)];
		//[self.delegate perform_string:@"fullscreen_dismiss"];
	}];
}
